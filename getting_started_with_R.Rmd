...
R evaluates the expression and assigns it the name `Pi`. `Pi` is now an _object_ in memory. R is case sensitive. "Pi" and "pi" are different.
...
- Example: the `sqrt` function takes the square root of a numeric object. The `Pi` object is the argument.
...
- Example: import a CSV file named `alb_homes.csv` and save as an object named `homes`:

```{r }
homes <- read.csv('alb_homes.csv')
```
...
- The function `read.csv('alb_homes.csv')` tells R to import a CSV file named 'alb_homes.csv'. Where does it look for it?
...
- You can also set it using the `setwd` function. For example, you might have something like the following at the top of a R script:
```{r eval=FALSE}
...
- Typing `?` before a function in the console and hitting Enter will take you to a help page for that function. (eg, `?nchar`)
- At the bottom there will usually be examples of how to use the function.
- The `example` function will run the examples for you so you can examine what the function does. (eg, `example(nchar)`)
